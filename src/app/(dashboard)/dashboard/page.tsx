...
interface DashboardData {
  ...
  kpis: {
    ...
    homologousMonthTotal: number
    monthChange: number
    isOngoingMonth: boolean
...
export default function DashboardPage() {
  ...
  const homologousLabelText = monthLabel(curMonth && curYear ? { year: curYear - 1, month: curMonth } : null)
  const changeLabel = data?.kpis.isOngoingMonth
    ? `vs 1-${new Date().getDate()} ${homologousLabelText}`
    : `vs ${homologousLabelText}`
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
        {loading ? Array(7).fill(0).map((_, i) => <Skeleton key={i} className="h-28" />) : (
          <>
            <KpiCard
            ...
            />
            <KpiCard
              title="Vendas vs Orçamento"
              value={data?.kpis.targetPct !== null && data?.kpis.targetPct !== undefined ? `${data.kpis.targetPct.toFixed(0)}%` : '—'}
              subtitle={
                data?.kpis.thisMonthBudget
                  ? `${formatCurrency(data?.kpis.totalSalesMonth || 0)} de ${formatCurrency(data.kpis.thisMonthBudget)}`
                  : 'Orçamento não definido para este mês'
              }
              color={(data?.kpis.targetPct ?? 0) >= 100 ? 'green' : (data?.kpis.targetPct ?? 0) >= 70 ? 'blue' : 'red'}
